...
images_dir = '/exp1/ClassImbalanceExperiments/val/images'
...
#     dice_core = dice_loss_specific_class(y_true, y_pred, 1)  
#     dice_enhancing = dice_loss_specific_class(y_true, y_pred, 3) 
#     dice_edema = dice_loss_specific_class(y_true, y_pred, 2) 
...
model.compile(loss= lambda y_true, y_pred: hybrid_loss_1(alpha, beta, gamma, delta, y_true, y_pred), optimizer=keras.optimizers.Adam(learning_rate=0.001), metrics = ['accuracy',sm.metrics.IOUScore(threshold=0.5), dice_coef, precision, sensitivity, specificity, dice_coef_necrotic, dice_coef_edema ,dice_coef_enhancing] )
...
plt.savefig("hybrid_loss_1_prediction.png")
plt.close()